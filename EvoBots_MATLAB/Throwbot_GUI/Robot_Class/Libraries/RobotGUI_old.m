...
% handles    structure with handles and user data (see GUIDATA)
% s = serial('COM12');
s = Bluetooth('Evobot_36',1); %Setting the bluetooth port for each robot MARK
 
pause(3);
fopen(s);       %use fopen to connect to the bluetooth MARK
success=0;
'Connected'
handles.bt=s;
guidata(hObject,handles)
...
while(button_state)
    ...
    if (strcmp(temp{2},'Motor Command Received'))
    elseif(strcmp(temp{2},'Motor Command Complete'))
    elseif(strcmp(temp{2},'Obstacle Found'))
    else
        num = size(temp);
        pause(0.05);
        for i=2:num(2)-1
