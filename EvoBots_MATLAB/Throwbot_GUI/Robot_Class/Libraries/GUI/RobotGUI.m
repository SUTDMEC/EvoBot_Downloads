...
% Last Modified by GUIDE v2.5 05-Aug-2014 01:19:25
...
handles.MyStructHere = evalin('base','Map_Data');
handles.t = timer('TimerFcn',  'myfunct, handles',...%'{scatterbutton2_Callback}',...
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
current_data = varargin{1}.data.map.robot_path;
...
% --- Executes on key press with focus on pair_bt and none of its controls.
function pair_bt_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to pair_bt (see GCBO)
...
% --- Executes on button press in pair_bt.
function pair_bt_Callback(hObject, eventdata, handles)
% hObject    handle to pair_bt (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%     str=fscanf(bt); %get input from bt
...
%                 st=datestr(addtodate(handles.tset,seconds,'millisecond'),'HH:MM:SS:FFF');
...
%         current_y_position=(rel_distancematrix(2)/380)*sind(robot_orientation)+(rel_distancematrix(1)/380)*cosd(robot_orientation)+moved_y_outofscreen
        current_x_position = map_x(1);
        current_y_position = map_y(1);
