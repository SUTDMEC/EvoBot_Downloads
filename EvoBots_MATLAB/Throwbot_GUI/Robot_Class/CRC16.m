function [ checksum ] = CRC16( str )
%Implementation of CRC16 using polynomial 0x8005 - (X^16 X^15 X^2 1)
%based off of http://mbed.org/users/EmLa/code/CRC16/
%written by WenZheng Li

%usage:
%to generate checksum, pass in message as parameter
%to detect error, pass in messsage concatonated with checksum as parameter,
%result will be non-zero if there is error.

persistent SHIFTER;
persistent crc16table;

%lookup table for the polynomial
if isempty(crc16table)
    crc16table = [ 
    ...
    crc16table =    uint16(hex2dec(crc16table));
    SHIFTER =       uint16(hex2dec('00FF'));
end

%magic. google how CRC works if you care to know
crc =  uint16(0);
for i = 1 : length(str)
    crc = bitxor(bitshift(crc,8),crc16table(bitand(bitxor(bitshift(crc,-8),uint16(str(i))),SHIFTER)+1));
end

%return checksum as string of 2 characters
checksum = [char(bitshift(crc, -8)) char(bitand(crc, SHIFTER))];
return
end
